	...
	internal class Program
	{
		...
		private static void Game_OnGameLoad(EventArgs args)
		{
			...
			if (Spells.Count > 0)
			{
				...
				Config.SubMenu("Settings").AddItem(new MenuItem("cloneChecker", "Clone Checker").SetValue(true));
		...
		{
			if (Config.Item("cloneChecker").GetValue<bool>())
			{
				foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy && !hero.IsDead && hero.IsVisible && hero.IsValid))
				{
